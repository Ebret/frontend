...
describe('TestUsersList Component', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('renders the component with all test users', () => {
    render(<TestUsersList />);
  ...
  it('shows instructions for seeding users', () => {
    ...
    expect(screen.getByText(/These test users are for development purposes only/i)).toBeInTheDocument();
    // The text might be split across elements, so we'll check for parts of it
    expect(screen.getByText(/development purposes only/i)).toBeInTheDocument();
  });
});
