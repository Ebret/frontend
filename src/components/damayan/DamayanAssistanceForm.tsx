...
const DamayanAssistanceForm: React.FC = () => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const documentUrls: FileInfo[] = files.map(file => ({
        name: file.name,
          ...
          <div className="space-y-6">
            <div>
              ...
              <select
                id="fundId"
                name="fundId"
                className={`mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md ${errors.fundId ? 'border-red-300' : ''}`}
