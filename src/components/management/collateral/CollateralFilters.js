...
/**
 * Collateral Filters Component
 * 
 * Provides advanced filtering options for collateral management:
 * - Type filter
 * - Status filter
 * - Value range filter
 * - Expiry range filter
 ...
 */
const CollateralFilters = ({ filters, onFilterChange, onSaveFilter }) => {
  ...
  return (
    <div className="mt-4 bg-gray-50 p-4 rounded-md border border-gray-200">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        {/* Value Range Filter */}
        <div>
          <label htmlFor="valueRange" className="block text-sm font-medium text-gray-700">
            Value Range
          </label>
          <select
            id="valueRange"
          ...
          >
            ...
            <option value="low">Low (< ₱1,000,000)</option>
            <option value="medium">Medium (₱1,000,000 - ₱3,000,000)</option>
            <option value="high">High (> ₱3,000,000)</option>
          </select>
        </div>
        
        {/* Expiry Range Filter */}
        <div>
          <label htmlFor="expiryRange" className="block text-sm font-medium text-gray-700">
            Document Expiry
          </label>
          <select
            id="expiryRange"
            ...
            onChange={(e) => onFilterChange('expiryRange', e.target.value)}
            className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
          >
            <option value="">Any Expiry</option>
            <option value="next30">Expiring in 30 Days</option>
            <option value="next90">Expiring in 90 Days</option>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label htmlFor="loanToValue" className="block text-sm font-medium text-gray-700">
              Loan-to-Value Ratio
            </label>
            <select
              id="loanToValue"
              name="loanToValue"
              value={filters.loanToValue || ''}
              onChange={(e) => onFilterChange('loanToValue', e.target.value)}
              className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
            >
          ...
          <div>
            <label htmlFor="appraisalDate" className="block text-sm font-medium text-gray-700">
              Last Appraisal
            </label>
            <select
              id="appraisalDate"
              ...
              onChange={(e) => onFilterChange('appraisalDate', e.target.value)}
              className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
            >
...
export default CollateralFilters;
